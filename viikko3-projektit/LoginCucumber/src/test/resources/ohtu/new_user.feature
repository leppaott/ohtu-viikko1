Feature: A new user account can be created if a proper unused username and password are given
    ...
    Scenario: creation successful with correct username and password
        ...
        When  username "asd" and password "asdwfwf23" are entered
        Then  system will respond with "new user registered"

    Scenario: creation fails with correct username and too short password
        Given command new user is selected
        When  username "asd" and password "dd" are entered
        Then  system will respond with "new user not registered"

    Scenario: creation fails with correct username and password consisting of letters
        Given command new user is selected
        When  username "asd" and password "ddddddddd" are entered
        Then  system will respond with "new user not registered"

    Scenario: creation fails with too short username and valid password
        Given command new user is selected
        When  username "X" and password "ddddd323ddd" are entered
        Then  system will respond with "new user not registered"
